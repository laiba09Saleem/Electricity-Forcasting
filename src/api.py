...
def load_assets():
    global model, scaler
    logger.info("[STARTUP] System Startup: Loading ML Artifacts...")
    try:
        # Load Scaler
        ...
        if os.path.exists(Config.MODEL_PATH):
            ...
            model.eval()
            logger.info("[SUCCESS] LSTM Model loaded successfully.")
        else:
            logger.warning("[WARN] Model file not found at %s", Config.MODEL_PATH)
    except Exception as e:
        logger.error("[ERROR] Critical error during asset loading: %s", str(e))
...
def forecast(req: ForecastRequest):
    logger.info("[FORECAST] Forecast requested for %s (%s to %s)", req.country_code, req.start, req.end)
...
def eda(country_code: str, start: str, end: str):
    logger.info("[EDA] EDA requested for %s", country_code)
    try:
